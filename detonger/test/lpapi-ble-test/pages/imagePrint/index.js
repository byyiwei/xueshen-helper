...
Page({
     ...
     */
    data: {
        canvasId: "lpapi-image-print-test",
        labelWidth: 960,
        labelHeight: 960,
    ...
    async printLabel(pageComplete, preview) {
        ...
        const urlList = [
            // 静态图片打印测试。
            // 备注：如果需要打印资源中的静态图片，需要在当前页面中必须要有一个 type="2d"的canvas，哪怕不用，也必须存在，否则无法打印。
            "/static/yinlifun.png",
            // 网络图片打印测试
            "https://dothantech.com/test/images/QRCode_OfficialAccounts.png",
            // base64图片打印测试
        ]
        // 创建打印任务，初始化canvas大小。
        api.startJob({
        ...
        // 备注：如果需要绘制不同大小的标签，可以在 startJob之前进行循环。
        for (const item of urlList) {
            ...
            // 加载图片
            const image = await api.loadImage(item);
            api.drawImage({
                image: image,
                x: margin,
                y: margin,
